...
ARG network_name=sandbox
RUN awk '{if (index($1, "NETWORK_NAME") != 0) {print("NETWORK_NAME = \"$network_name\"")} else print($0)}' /etc/indy/indy_config.py> /tmp/indy_config.py
RUN mv /tmp/indy_config.py /etc/indy/indy_config.py

ARG pool_ip=127.0.0.1

RUN generate_indy_pool_transactions --nodes 4 --clients 5 --nodeNum 1 2 3 4 --ips="$pool_ip,$pool_ip,$pool_ip,$pool_ip" --network=$network_name
